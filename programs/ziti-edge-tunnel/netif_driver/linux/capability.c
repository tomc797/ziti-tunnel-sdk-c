...
ziti__cap_assert(unsigned long linux_cap_mask, unsigned long flags)
{
    ...
    if (sys_rc < 0) {
        ...
        ZITI_LOG(ERROR, "failed to set thread capabilities: (%d) %s",
            -saved_errno, strerror(saved_errno));
        return -saved_errno;
    }

    if (!thread_state.armed)
        thread_state.saved_cap = oldcap;
...
ziti__cap_restore(void)
{
    ...
    if (!thread_state.armed) {
        ZITI_LOG(ERROR, "no previously saved capabilitity to restore");
        return -EINVAL;
    }
